...
CONSUL_CERT=$(aws ssm get-parameter --name /${CLUSTER_TAG_VALUE}/certificates/client-cert --with-decryption | jq '.Parameter.Value' | tr -d '"')
CONSUL_KEY=$(aws ssm get-parameter --name /${CLUSTER_TAG_VALUE}/certificates/client-key.pem --with-decryption | jq '.Parameter.Value' | tr -d '"')
